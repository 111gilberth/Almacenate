...
@section('content')
                        ...
                        <div class="form-group" id="clidv">
                            ...
                            <select name="idcliente" class="form-control selectpicker" id="idcliente" required data-live-search="true">
                                @foreach($personas as $persona)
                                    <option {{($persona->nombre == 'Consumidor final') ? "selected":""}} value="{{$persona->idpersona}}">{{$persona->nombre}}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                            ...
                            <div class="panel-body">
                                ...
                                <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <input type="number" value="1" step=".01" name="pcantidad" id="pcantidad"
                                               class="form-control">
                                    </div>
                                </div>
                                <div class="col-lg-2 col-md-2 col-sm-2 col-xs-12">
                                    <div class="form-group">
                                        ...
                                        <input type="number" step=".01"  name="pstock" id="pstock"
                                               class="form-control">
                                    </div>
                                </div>
                        ...
                        <div class="panel panel-primary">
                            <div class="panel-body">
                                ...
                                <div class="form-group" id="pa3">
                                    ...
                                    <input type="number" form="cre" class="form-control" onchange="calcularVuelto()" value="0" id="paga"
                                           placeholder="Efectivo" min="0" name="paga">
                                </div>
                            </div>
                        </div>
                        <div class="form-group">
                                    <h4 style="font-weight: 900;" id="vuelto"> Devolver: $ 0.00</h4>
                                </div>
    ...
    @push('scripts')
        ...
        <script>
            ...
            function actualizar_precio(m){
                
                
                var pv = document.getElementsByName('precio_venta[]');
                var cant = document.getElementsByName('cantidad[]');
                var desc = document.getElementsByName('descuento[]');
                var pvv=pv[m].value;
                var cantv=cant[m].value;
                var descv=desc[m].value;

                subtotal[m]=pvv*cantv-descv;
                total=0
                i=0
                document.getElementById('subtotal['+m+']').innerHTML = subtotal[m];
            ...
            function calcularVuelto(){
               ...
               var pago=parseFloat($('#paga').val());
               
               if (total < pago){
                 var vuelto=pago-total;
                 
                 $('#vuelto').html("Devolver: $" + vuelto);
                 $('#vuelto').show();
               }
            ...
            function agregar() {
                ...
                if (idarticulo != "" && cantidad != "" && cantidad > 0 && pdescuento != "" && precio_venta != "") {
                    if (stock_numero >= stock_cantidad) {
                        ...
                        var fila = '<tr class="selected" id="fila' + cont + '"><td><button type="button" class="btn btn-danger btn-xs" onclick="eliminar(' + cont + ');">X</button></td><td><input type="hidden" name="idarticulo[]" value="' + idarticulo + '">' + articulo + '</td><td><input type="number"  id="cantidad[]" name="cantidad[]" value="' + cantidad + '" onchange="actualizar_precio('+cont+')" ></td><td><input readonly type="number"  name="precio_venta[]" value="' + precio_venta + '"></td><td><input readonly type="number" name="descuento[]" value="' + descuento +'"></td><td><p id= "subtotal['+cont+']" name="subtotal['+cont+']">'+ subtotal[cont] +'</p></td></tr>';
                        cont++;
                        limpiar();
                        $('#total').html("$ " + total);
                        $('#total2').html("Total Compra: $" + total);
